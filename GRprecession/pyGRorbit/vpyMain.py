class vector6d:
    ...
    def __str__(self):
        ...
        return '({:g},{:g},{:g},{:g},{:g},{:g})'.format(self.x, self.y,self.z, self.u, self.v,self.w)

    def __repr__(self):
        """Unambiguous string representation of the vector."""
        return repr((self.x, self.y, self.y, self.u, self.v, self.w))

    def dot(self, other):
        """The scalar (dot) product of self and other. Both must be vectors."""
        if not isinstance(other, vector6d):
            raise TypeError('Can only take dot product of two vector6d objects')
        return self.x * other.x + self.y * other.y + self.z * other.z + self.u * other.u + self.v * other.v + self.w * other.w
 
    def __sub__(self, other):
        """Vector subtraction."""
        return vector6d(self.x - other.x, self.y - other.y, self.z - other.z, self.u - other.u, self.v - other.v, self.w - other.w)
    ...
    def __mul__(self, scalar):
        ...
        if isinstance(scalar, int) or isinstance(scalar, float):
            return vector6d(self.x*scalar, self.y*scalar, self.z*scalar, self.u*scalar, self.v*scalar, self.w*scalar)
        raise NotImplementedError('Can only multiply vector6d by a scalar')
    ...
    def __pow__(self, scaler):
        return vector6d(self.x**scaler, self.y**scaler, self.z**scaler, self.u**scaler, self.v**scaler, self.w**scaler)

    def __neg__(self):
        """Negation of the vector (invert through origin.)"""
        return vector6d(-self.x, -self.y, -self.z, -self.u, -self.v, -self.w)
...
def gnp2vec(array): #Transfer "general" coordinate ndarray to two vpython vectors
        vec1 = vec(array.x, array.y, array.z)
        vec2 = vec(array.u, array.v, array.w)
        return vec1, vec2
...
def f(t,y):
        ...
        F4 = F * (-y.y/r)
        F5 = F * (-y.z/r)
        return vector6d(F0,F1,F2,F3,F4,F5)
...
# ==============================================================
# intial matrix
Y0 = vector6d(r0,0,0,0,v0,0) # normal settinf
Y0 = vector6d(r0,0,0,0,0,v0) # !!! Since WebGL is (z,x,y,vz,vx,vx)!!!
Y = Y0
t = 0
...
Planet.pos = vec(Y.x, Y.y, Y.z)
Planet.radius = Rplanet
Planet.v = vec(Y.u, Y.v, Y.w)
...
def Camera_trace(t):
        w = vec(0.05,0.05,0.05)* 0.8
        scale = 1
        x = scale*cos(w.x*t)
        y = scale*sin(w.y*t)+0.5
        z = scale*sin(w.z*t)
        return x,y,z
...
def inislider():
        ...
        if center_of_camera == "Planet":
                ...
                scene.camera.axis = Solar.pos - scene.camera.pos
        button_run.text = '<i id="run"><i>'
        Y = vector6d(r0,0,0,0,0,v0) # !!! Since WebGL is (z,x,y,vz,vx,vx)!!!
        X, V = gnp2vec(Y)
        Planet.pos = X
        Planet.v = V
        Planet.clear_trail()
