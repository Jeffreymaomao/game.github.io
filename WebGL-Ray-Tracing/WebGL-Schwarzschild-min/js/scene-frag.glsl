...
// color 
vec3 calculateColor(float rad, float phi, float distance, vec3 backgroundColor){
    float noise_rad_phi = noise(vec3(40.0*rad+cos(phi)));
...
vec3 RayMarching(Ray ray){
    ...
    for(int STEP=0; STEP<MAX_STEPS; STEP++){
        ...
        if( r > MIN_RADIUS && r < MAX_RADIUS && pos.z*pos_last.z < 0.0){
            if(pos.z > 0.0){
                color = hsv2rgb(vec3(phi/TWO_PI, 1.0, 0.3));
                return color;
            }
            else if(pos.z<0.0){
                color = hsv2rgb(vec3(phi/TWO_PI, 0.5, 1.0));
                return color;
            }
    ...
    if(drawDisk>0 && isHit){
        ...
        color = calculateColor(hitRad, hitPhi, hitDistance, backgroundColor);
        color = color * 0.5 + 0.5;;
    }else if(isInside){
