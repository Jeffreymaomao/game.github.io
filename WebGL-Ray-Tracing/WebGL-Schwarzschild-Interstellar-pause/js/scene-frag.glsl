...
float MIN_RADIUS = 3.0*RS; // Schwarzschild Radius
float MAX_RADIUS = 9.0*RS; // Schwarzschild Radius
...
vec3 interpolate3Colors(float r) {
    vec3 result;
    vec3 c1 = vec3(204.0,85.0,70.0);// rgb(204,85,70)
    vec3 c2 = vec3(230.0,155.0,65.0);// rgb(230,155,65)
    vec3 c3 = vec3(255.0,255.0,255.0);// rgb(255,255,255)
...
// color 
vec3 calculateColor(vec3 pos, float rad, float phi, float distance, vec3 backgroundColor){
    // if(pos.z>0.0){ return vec3(1.0);}
    // else if(pos.z<0.0){ return vec3(0.5);}
    rad = clamp(rad, 0.0, 1.0);
    float n = noise(vec3(40.0*rad));
    float color = n;
    color *= exp(-distance * 0.01);
    color += 0.9 * rad;
    vec3 diskColor = interpolate5Colors(rad+0.2*n) * 0.6 +  1.0 * color;
    diskColor = adjustSaturation(diskColor, 2.0);

    // 添加模糊效果
    float alpha = rad*n;

    float tmp1 = rad - 0.6 * mod(phi, TWO_PI)/TWO_PI;
    float tmp2 = rad - 0.6 * mod(phi + 0.25*TWO_PI, TWO_PI)/TWO_PI;
    float tmp3 = rad - 0.6 * mod(phi + 0.5*TWO_PI, TWO_PI)/TWO_PI;
    float tmp4 = rad - 0.6 * mod(phi + 0.75*TWO_PI, TWO_PI)/TWO_PI;
    float width = 1e4;
    float tmp = exp(-width*tmp1*tmp1) + exp(-width*tmp2*tmp2) + exp(-width*tmp3*tmp3) + exp(-width*tmp4*tmp4);
    alpha *= (1.0 - tmp * (1.0-rad));
    alpha += 0.1 * rad;
...
vec3 RayMarching(Ray ray){
    ...
    for(int STEP=0; STEP<MAX_STEPS; STEP++){
        ...
        phi += iTime * 0.05;
        float n = noise(vec3(10.0*cos(phi)*rad));
        float n1 = noise(vec3(20.0*cos(phi)*rad));
        float Rnoise = 0.3 * n1;
        float Znoise = 0.005 * rad * (n+n1);
        if( r > MIN_RADIUS && r < MAX_RADIUS - Rnoise && pos.z*pos_last.z < Znoise){
            rad -= 0.01 * noise(vec3(40.0*phi, 30.0*rad, 1.0));
            if(!isHit){
                hitRad = 1.0-rad;
                hitPhi = phi;
            }
    ...
    if(drawDisk>0 && isHit){
        ...
        color = calculateColor(pos, hitRad, hitPhi, hitDistance, backgroundColor);
        color = color * 0.5 + 0.5;
    }else if(isInside){
...
void main() {
    ...
    float cameraFOV = PI / 5.0; // Field of View (FoV)
