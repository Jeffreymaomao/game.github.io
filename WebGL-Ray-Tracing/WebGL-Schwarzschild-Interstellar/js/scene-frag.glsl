...
vec3 calculateColor(vec3 pos, float rad, float distance, vec3 backgroundColor){
    ...
    float n = noise(vec3(30.0*rad));
    float color = n;
    color *= exp(-distance * 0.01);
    color += 1.5*rad;
    color = clamp(color, 0.0, 0.8);
    vec3 diskColor = interpolate5Colors(sqrt(rad)) * 0.6 +  1.0*color;
    diskColor = adjustSaturation(diskColor, 2.0);

    // 添加模糊效果
    float edgeBlur = smoothstep(0.9, 1.0, rad); // 在边缘处平滑插值

    // 使用 edgeBlur 调整 alpha 值以实现模糊效果
    float alpha = sqrt(rad*n) * (1.0 - edgeBlur) * (n+0.5);
...
vec3 RayMarching(Ray ray){
    ...
    for(int STEP=0; STEP<MAX_STEPS; STEP++){
        ...
        phi += iTime * 0.05;
        float n = noise(vec3(30.0*cos(phi)));
        float Znoise = 0.05 * rad * n;
        float Rnoise = 0.4 * n * n;
        if( r > MIN_RADIUS && r < MAX_RADIUS - Rnoise && pos.z*pos_last.z < Znoise * Rnoise){
    ...
    vec3 backgroundColor = texture2D(uSkysphere, texCoords).rgb * 0.499;
    if(drawDisk>0 && isHit){
