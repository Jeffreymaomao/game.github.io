...
function initWebGL(canvas, vertCode, fragCode) {
    ...
    function render() {
        ...
        const distanceLocation = gl.getUniformLocation(shaderProgram, 'distance');
...
function mouseEvent(canvas){
    ...
    canvas.addEventListener('mousemove', (e) => {
        if (config.mouse.down) {
            ...
            config.camera.polarAngle -= 0.01 * config.mouse.deltaX;
            config.camera.azimuthalAngle -= 0.01 * config.mouse.deltaY;
